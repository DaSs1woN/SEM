...
def atom_detection(ori_img):
	height=28
	width =28
	channels=1
	data = np.zeros(shape=(1,height*width),dtype=np.int64)
	if ori_img is None:
		print("No Pictures")
		exit(0)
	img=cv2.resize(ori_img,(height,width))
	img=img.reshape([height*width])
	data[0,:]=img
	data=data/255.0
	atom_classifier=tf.estimator.Estimator(
			model_fn=cnn_model_fn,
			model_dir="models/atom_classify_twotypes_model")
